...
async function getReserves () {
    ...
    const res = await fetch(`https://pf-eclipseroyale-qd2v.onrender.com/reservation/getAllReservations?page=1&limit=100&unique=${uniqueParam}`,{
        method: "GET",
        headers: {
...
async function ReservesPending ({params}:any) {
    ...
    const reserves_paid = reserves.filter((reserves:{price:number}) => reserves.price >1000)
    const cont = reserves_paid.length

    const RenderPaginationButtons = () => {
      const buttons = [];
      const totalPages = Math.ceil(cont/40)
  
      for (let i = 1; i <= totalPages; i++) {
        buttons.push(
          <Link key = {i} href={`/reservation/getAllReservations?page=${i}&limit=40&unique=${uniqueParam}`}>
